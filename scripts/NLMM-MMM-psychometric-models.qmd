...
subtitle: "An applied overview and introduction"
...
opts_chunk$set(
               ...
               cache=TRUE, 
               cache.extra="quarto-native-output",
               size="footnotesize",
...
my_hypotheses <- function(model, prefix= "eta", link_lambda = "identity") {
  if (link_lambda == "logit") 
    cat("NB: converting estimate and CIs for lambda into proportion space.\n",
        "The SE of the estimate, however, is in logits (since it cannot",
        "meaningfully be expressed in proportion space).")
  ...
  hypothesis(model, paste0(prefix, "_Size > 0"))$hypothesis %>%
    ...
    { if (paste0("b_", prefix,
                 "_ConditionCrowded.vs.Uncrowded") %in% variables(model)) 
      rbind(
        ., 
        rbind(    
          hypothesis(model,
                     paste0(prefix,
                            "_ConditionCrowded.vs.Uncrowded < 0"))$hypothesis,
          hypothesis(model,
                     paste0(prefix,
                            "_Size:ConditionCrowded.vs.Uncrowded < 0"))$
          hypothesis) %>%
          mutate(Hypothesis = c("$\\beta_{Crowded}$ < 0",
                                "$\\beta_{Size:Crowded}$ < 0"))) else . } %>%
    { if (any(c("b_lambda_Intercept",
                "b_lapse_Intercept") %in% variables(model))) 
      rbind(
        ., 
        hypothesis(
          model, 
          paste0(
            intersect(
              ...
              variables(model)), " < ",
            if (link_lambda == "logit") qlogis(lambda_h) else lambda_h), 
...
As of 2025, toolboxes/libraries for psychometric models include:
...
\begin{align}
p(response | stimulus, \alpha, \beta, lapse, bias) = \\
(1-lapse) * p(response | stimulus, \alpha, \beta) + lapse * bias
\end{align}

Under this second formulation, the floor performance is $lapse * bias$ and the ceiling performance is $1 - (lapse - lapse * bias) = 1 - (1 - bias) * lapse$. While the *lapse* and *bias* parameters of this formulation are not the same as the $\lambda$ and $\gamma$ parameters of the Wiechmann and Hill formulation, the parameters can be translated into each other:^[This [interactive ShinyApp](https://hlplab.shinyapps.io/BayesianIdentificationAndDiscrimination/) illustrates the effect of lapse rates in the context of an ideal observer over two Gaussian categories. The app's first tab describes category identification (categorization) and perceptual discrimination for two Gaussian categories by an ideal observer. Have a look at the "Ideal identification" plot (right side, in the middle of the lower half of the screen) and how it changes if you changes as a function of the rate of attentional lapsing (top left) and bias (unclick "Prior as bias" and then try out different biases). The ideal categorization function between two Gaussian categories turns out to be a logistic function (with linear and quadratic stimulus effects)---i.e., one of the psychometric functions mentioned above (and a GLM). So the effects of lapse rate and bias give you an initial idea of how these parameters can affect the fit of psychometric functions.] 
...
## Data
We use the same data as in the GLMM tutorial from Ashley Clark's (as of 2022 unpublished) experiment on foveal crowding^[Ashley Clark's experiment on foveal crowding. Do not share this data without Ashley's permission.]. The goals of Ashley's research are to (1) investigate the effect of crowding within the foveola, and (2) examine if and how fixational eye movements (FEMs) influence crowding at this scale. Based on previous research, we hypothesize that crowding will be detrimental to foveal vision, as it is in peripheral vision, but on a finer scale. Further, based on the recent findings that FEMs are beneficial for high-acuity vision, I expect a relationship between FEMs and the strength of crowding within the foveola, with larger and less precise FEMs increasing the negative effects visual crowding.
...
d %<>%
  ...
  select(Subject, Condition, Threshold.Subj, DiffusionConstant.Subj,
         Area.Subj, Span.Subj, Speed.Subj, everything(), Span, Speed,
         Curvature) %>%
...
As a first illustration, here is the formula describing a psychometric model with a logistic link function for the case of non-hierarchical data (data from a single subject). `brms` also allows us a more elegant mixture formulation (see appendix and replies to \url{https://discourse.mc-stan.org/t/fitting-lapsing-psychometric-functions-with-brms/5762/2}) but the non-linear model implementation following Wiechmann and Hill (2001) is more transparent for the present purpose:

```{r, echo=T}
# brms formula for the non-linear model.
# It has 3 components: lapse rate, guess
# (bias), and linear predictor eta, each
# described by a linear predictor. Here,
# eta is intercept + effect of stimulus;
# the other two are just 'intercepts'.
BF <- brmsformula(
  ResponseCorrect ~ gamma +
    (1 - gamma - lambda) *
    inv_logit(eta),
  eta ~ 1 + Size,
  gamma ~ 1,
...
```{r, echo=T}
# Store mean and sd of Size to transform
# model predictions back to original scale
Size.mu <- mean(d$Size)
Size.sd <- sd(d$Size)

# Standardize Size and set factor levels
d %<>%
    mutate(
      Condition = factor(
        Condition,
...
contrasts(d$Condition) <-
  cbind("Crowded.vs.Uncrowded" = c(-.5, .5))
```

## Fitting the model

We fit the model to the data from Subject 1 with the function `brm()` from the library `brms`, and summarize the output:

```{r, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=FALSE}
fit.lambda0.1 <- brm(
  BF.lambda,
  data = d %>%
    filter(Subject == "1"),
...
We can test the hypotheses we hold about the various parameters in the model using Bayesian hypothesis testing, implemented via the function `hypothesis` foorm the same package:

```{r}
my_hypotheses(fit.lambda0.1)
```

And, finally, the model fit against the data and the predictions of the model along with 95\% uncertainty intervals are shown in @fig-simple-example-fit.

```{r}
#| label: fig-simple-example-fit
#| fig-cap: "Model fit for Subject 1 with estimated lapse rate $\\lambda$ constrained to $[0, 0.5]$."
p <- suppressMessages(suppressWarnings(
  plot(conditional_effects(fit.lambda0.1), 
       plot = F, points = T, 
...
The threshold calculation at 62.5% accuracy is illustrated in @fig-simple-example-threshold.

```{r, warning=F}
#| label: fig-simple-example-threshold
#| fig-cap: "Determining the subject-specific threshold at 62.5% accuracy."
p + 
  p.common +
  geom_segment(x = -0.2645872, xend = -0.2645872, y = 0, yend = .625,
...
for (i in 1:length(par.lambda)) {
  for (j in 1:length(par.gamma)) {
    f = paste("accuracy ~", par.gamma[j], "+",
              1-par.gamma[j]-par.lambda[i], "* inv_logit(eta)")
    
    analyses[[paste(par.lambda[i], par.gamma[j])]] <- brm(
      brmsformula(
        f,
        # Change to use s_stimulus if the gelman scaled stimulus is desired.
        # This will mean the intercept is the estimate at the
        # center of the data.
        eta ~ 1 + stimulus,
        family = bernoulli(link="identity"),
        nl = TRUE),
      ...
      chains = 0,
      prior = my.priors,
      file = paste0("../models/sim-model-gamma", par.gamma[j],
                    "-lapse", par.lambda[i]))
  }
}
```

Here is an example draw from the data generator with 100 data points per stimulus at five different stimulus locations between 0 and 3 for $\lambda = .05$. The fit under the assumption of zero lapses is shown in @fig-sim-fit-nolapse. The ground truth functional relation including the lapse rate is shown in @fig-sim-ground-truth. Finally, the perceptual model comparison between the ground truth ($\lambda = .05$, gray) and the zero-lapse assumption (blue) is shown in @fig-sim-perceptual-only.


```{r, warning=FALSE}
#| label: fig-sim-fit-nolapse
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Example simulated psychometric data and fit under
#|   the assumption of zero lapses."
# without lapse
d.temp %>%
  ...
  theme_bw()
ggsave(file = "../figures/simulation-example-psychometric-fit-noLapse.png",
       width = 5, height = 3.5)
```

```{r, warning=FALSE}
#| label: fig-sim-ground-truth
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Simulated psychometric function showing ground truth
#|   parameters and comparison fit."
p <- 
  d.temp %>%
  ggplot(aes(x = stimulus, y = accuracy)) +
  ...
  annotate(geom = "text", 
           label = expression(gamma + (1 - gamma - lambda) *
                              logit^{-1} * (alpha + beta * stimulus)), 
           x = 3, y = 0, hjust = 1, color = "darkgray",
           fontface = "bold", size = 4) +
  annotate(
    geom = "text", 
    label = deparse(bquote(lambda == .(true.lambda))), parse = TRUE,
    x = 3, y = 1 - true.lambda - .05, hjust = 1, color = "darkgray", size = 4) +
  annotate(
    geom = "text", 
    label = deparse(bquote(gamma == .(true.gamma))), parse = TRUE,
    x = 3, y = true.gamma - .05, hjust = 1, color = "darkgray", size = 4) +
  scale_x_continuous("stimulus", breaks = -3:3) +
  scale_y_continuous(limits = c(0,1)) +
  theme_bw()
ggsave(p, file = "../figures/simulation-example-just-ground-truth.png",
       width = 5, height = 3.5)
...
p +
  geom_line(
    ...
    aes(
      ...
      group = group),
    color = "blue")
ggsave(file = "../figures/simulation-example-psychometric-fit.png",
       width = 5, height = 3.5)
```

```{r, warning=FALSE}
#| label: fig-sim-perceptual-only
#| fig-cap: "Comparison of the true perceptual model and the
#|   estimate obtained under the assumption of zero lapses."
estimates <- suppressMessages(suppressWarnings(
  update(analyses[[1]], newdata = d.temp,
...
tibble(stimulus = seq(0, 3, length.out = 100)) %>% 
 ggplot(aes(x = stimulus)) +
  ...
  geom_line(
    ...
    color = "blue") +
  annotate(geom = "text", label = "ground truth", x = 1.5, y = .75,
           hjust = 1, color = "darkgray", size = 4) +
  annotate(geom = "text", label = "estimate under\nassumption of 0 lapses",
           x = 1.5, y = .4, hjust = 0, color = "blue", size = 4) +
  scale_x_continuous("stimulus", breaks = -3:3) +
  scale_y_continuous("accuracy\n(when responding based on stimulus)",
                     limits = c(0,1)) +
  theme_bw()
ggsave(
  file = "../figures/simulation-example-psychometric-fit-just-responses-based-on-stimulus.png",
       width = 5, height = 3.5)
```
...
```{r simulation-loop, include=FALSE, cache=FALSE}
if (file.exists("../models/simulation.RData")) {
          ...
          for (n in par.n_per_stimulus) {
            ...
            for (s in par.stims) {
              for (k in 1:par.n_simulations) {
                d.temp = generate_data(
                  intercept = true.intercept, slope = true.slope,
                  gamma = true.gamma, lambda = true.lambda, 
                  stimuli = s, n_per_stimulus = n)
                
                # analysis assumptions
                for (i in 1:length(par.lambda)) {
                  for (j in 1:length(par.gamma)) {
                    print(paste("simulation:", k, "; analysis:",
                                paste(par.lambda[i], par.gamma[j]),
                                "; stimuli:", paste(s, collapse = ",")))

                    print(paste("true intercept:", true.intercept,
                                "; slope:", true.slope))
                    ...
                    d.sim %<>%
                      rbind(
                        model %>%
                          ...
                          mutate(simulation = k,
                                 model.lambda = par.lambda[i],
                                 model.gamma = par.gamma[j],
                                 n_per_stimulus = n,
                                 stimuli = paste(s, collapse = ","),
                                 true.intercept = true.intercept,
                                 true.slope = true.slope,
                                 true.lambda = true.lambda,
                                 true.gamma = true.gamma,
                                 divergent = model %>% nuts_params() %>%
  ...
  d.sim %<>%
    distinct(simulation, model.lambda, model.gamma, n_per_stimulus, stimuli, 
             true.intercept = true.intercept, true.slope = true.slope,
            true.lambda = true.lambda, true.gamma = true.gamma, term, .keep_all = T) %>%
...
The correlation among the intercept and slope estimates across simulation runs is shown in @fig-sim-corr-intercept-slope and @fig-sim-corr-grid.

```{r, warning=FALSE}
#| label: fig-sim-corr-intercept-slope
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Correlation between intercept and slope estimates
...
p <-
  ...
  scale_color_discrete(
    ...
    labels = c("low+mid+high performance", "mid performance",
               "mid+high performance")) +
  theme_bw()
ggsave(p, file = "../figures/simulation-correlation-intercept-slope.png",
       height = 4, width = 6)
p
```

```{r, warning=FALSE}
#| label: fig-sim-corr-grid
#| fig-cap: "Correlation between intercept and slope estimates
#|   across true parameter values and stimulus distributions."
p +
  (d.sim %>% 
     pivot_wider(
    values_from = c(Estimate, Est.Error, Q2.5, Q97.5),
...
ggsave("../figures/simulation-correlation-intercept-slope.png",
       height = 8, width = 9)
```

### Visualizing bias in the estimates

In the following plots, the gray dashed lines indicate the ground truth. The distribution of estimates for the intercept and slope obtained for the different simulation runs is summarized in @fig-sim-bias-uniform (uniform stimulus scheme) and @fig-sim-bias-all (all stimulus schemes).

```{r, warning=FALSE}
#| label: fig-sim-bias-uniform
#| fig-cap: "Bias in parameter estimates as a function of assumed lapse rate $\\lambda$ for uniform stimulus spacing."
p <- d.sim %>%
  filter(
    true.slope == 3,
  ...
  scale_x_continuous(
    "lambda assumed in analysis\n(gamma is set to ground truth chance)") +
  scale_color_discrete(
    "stimulus distribution\nacross performance",
    breaks = levels(factor(d.sim$stimuli)),
    labels = c("low+mid+high performance", "mid performance",
               "mid+high performance")) +
  facet_grid(term ~ true.intercept, scales = "free_y",
             labeller = "label_both") + 
  theme_bw()

ggsave(p, file = "../figures/simulation-bias-uniform.png",
       height = 8, width = 9)
p
```

```{r, warning=FALSE}
#| label: fig-sim-bias-all
#| fig-cap: "Bias in parameter estimates as a function of assumed lapse rate $\\lambda$ across all stimulus distributions."
d.sim %>%
filter(
    true.slope == 3) %>%
  group_by(model.lambda, model.gamma, n_per_stimulus, stimuli) %>%
...
ggsave("../figures/simulation-bias-of-fixed-lambda-analyses-true-slope3.png",
       height = 8, width = 9)
```
...
The first model assumes $\lambda = 0$ (no lapses). As shown in @fig-s3-lapse0, this model seems to present some evidence for an interaction between the crowdedness condition and the effect of stimulus size:

```{r, echo=T}
BF <- brmsformula(
  ResponseCorrect ~ .25 +
    (1 - .25 - 0) * inv_logit(eta),
...
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Model fit for Subject 3 assuming $\\lambda = 0$."
p <- plot(
  conditional_effects(
    fit.nl.s3.lambda0,
...
The next two models assume non-zero lapse rates of $\lambda = .05$ and $\lambda = .2$, shown in @fig-s3-lapse05 and @fig-s3-lapse02, respectively. Note how the strength of the evidence for an interaction systematically becomes weaker as we increase $\lambda$. Of course, we do not know the ground truth, but it is worth noting that Subject 3 does not seem to achieve performance close to 100% accuracy even for the largest stimuli tested (there are both accurate and inaccurate answers at the largest size values in the crowded condition). Indeed, we will see in the next section that the *estimated* $\lambda$ is close to 25%.

```{r}
BF <- brmsformula(
  ResponseCorrect ~ .25 +
    (1 - .25 - 0.05) * inv_logit(eta),
  eta ~ 1 + Size * Condition,
  family = bernoulli(link = "identity"),
  nl = TRUE) 
```
...
```{r, message=FALSE, warning=FALSE}
#| label: fig-s3-lapse05
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Model fit for Subject 3 assuming $\\lambda = 0.05$."
p <- plot(
  conditional_effects(
...
```{r, message=FALSE, warning=FALSE}
#| label: fig-s3-lapse02
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Model fit for Subject 3 assuming $\\lambda = 0.20$."
p <- plot(
  conditional_effects(
    fit.nl.s3.lambda0.2,
    conditions = make_conditions(d, "Condition")), 
...
```{r, message=FALSE, warning=FALSE}
#| label: fig-s3-lapse-inferred
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Model fit for Subject 3 with $\\lambda$ inferred from data."
p <- plot(
  conditional_effects(
    fit.nl.s3.lambda,
    conditions = make_conditions(d, "Condition")), 
...
BF.lambda.mixed <- brmsformula(
  # fit lapses in log-odds space
  # convert lapses to proportion below
  ResponseCorrect ~ .25 +
    (1 - .25 - inv_logit(lambda)) *
    inv_logit(eta),
  eta ~ 1 + Size * Condition +
    (1 + Size * Condition | g | Subject),
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=FALSE}
# lambda is unconstrained here.
# Lower (lb) & upper (ub) bounds
# can be set in priors.
my.priors.lapse <- c(
  prior(student_t(3, 0, 2.5),
        class = "b", nlpar = "eta"),
  prior(student_t(3, 0, 2.5),
        class = "b", nlpar = "lambda"), 
  prior(cauchy(0, 2.5),
        class = "sd", nlpar = "eta"),
  prior(cauchy(0, 2.5),
        class = "sd", nlpar = "lambda"))
...
get_samples_from_nonlinear_model <- function(
  ...
  model %>% 
    ...
    mutate(
      ...
      b_Condition = b_eta_ConditionCrowded.vs.Uncrowded +
        ConditionCrowded.vs.Uncrowded,
      b_SizeCondition = `b_eta_Size:ConditionCrowded.vs.Uncrowded` +
        `Size:ConditionCrowded.vs.Uncrowded`
    ) %>%
    ungroup() %>%
...
p <- suppressMessages(suppressWarnings(plot_psychometric(model = fit.lambda.mixed, animate = F))) 
...
```{r, include=FALSE, fig.width=8, fig.height=14, fig.show='hold', dev='png', warning=F, message=F}
...
\begin{figure}[H]
\centering
\animategraphics[width=0.8\linewidth,controls,loop]{10}{NLMM-MMM-psychometric-models_files/figure-pdf/lambda-animation-frames-}{1}{100}
\caption{Posterior draws for the hierarchical psychometric model. Each frame shows one posterior sample and the corresponding subject-specific predictions.}
\end{figure}
```

## Visualizing correlations between $\lambda$ and the parameters of the perceptual model

The joint posterior distribution of the model's fixed effect parameters $\alpha$, $\beta$, and $\lambda$ is shown in @fig-3d-lambda-correlations. This reveals a correlation between the three parameters. Similar plots could be obtained for any combination of parameters, including the subject-specific estimates of any parameter (`shinystan::launch_shinystan()`) provides an interactive HTML graphical user interface that lets you investigate any such correlations; you might know `R`'s Shiny apps from [webpages like this reporting COVID statistics](https://mappingmonroe.maps.arcgis.com/apps/opsdashboard/index.html#/217749730f174776a3896b3e8950e03b)).

```{r, warning=FALSE}
#| label: fig-3d-lambda-correlations
#| column: margin
#| fig-width: 4
#| fig-height: 4
#| fig-cap: "Joint posterior distribution of fixed effect parameters $\\alpha$, $\\beta$, and $\\lambda$."
d.draws <- 
  fit.lambda.mixed %>% 
  spread_draws(
    b_eta_Intercept,
...
scatterplot3d(
  d.draws$b_eta_Intercept, d.draws$b_eta_Size, d.draws$b_lambda_Intercept,
  pch = 16, angle = 55, color = adjustcolor("black", alpha.f = .015),
  xlab = "b_eta_Intercept", ylab = "b_eta_Size", zlab = "b_lambda_Intercept")
```
...
fit.lapse.mixed.mixture <- brm(
  bf(ResponseCorrect ~ 1, 
     mu1 ~ 1 + (1 | Subject),
     mu2 ~ 1 + Size * Condition +
       (1 + Size * Condition | Subject), 
     theta1 ~ 1 + (1 | Subject)),
  data = d, 
  family = mixture(bernoulli("logit"),
                   bernoulli("logit"),
                   order = "none"),
  init = 0,
  control = list(adapt_delta = 0.999),
...
get_samples_from_mixture_model <- function(
  ...
  model %>% 
    ...
    mutate(
      ...
      b_mu2_Condition = b_mu2_ConditionCrowded.vs.Uncrowded +
        ConditionCrowded.vs.Uncrowded,
      b_mu2_SizeCondition = `b_mu2_Size:ConditionCrowded.vs.Uncrowded` +
        `Size:ConditionCrowded.vs.Uncrowded`
    ) %>%
    ungroup() %>%
    mutate_at("Subject", factor) %>%
...
Next, we visualize the joint posterior distribution of the parameters of the model. The joint posterior distribution of the lapse rate (mixture 1 weight), bias (mixture 2 mean), and mean of the psychometric model (mixture 2) is shown in @fig-3d-mixture-correlations. The joint posterior distribution of $\gamma$, $\lambda$, and the psychometric model intercept is shown in @fig-3d-gamma-lambda. Most parameter values lie very close to the boundary at 0, illustrating why sampling $\gamma$ and/or $\lambda$ directly is difficult. The lapse rate vs. mean and slope of the psychometric model are shown in @fig-3d-lapse-slope.

```{r, warning=FALSE}
#| label: fig-3d-mixture-correlations
#| column: margin
#| fig-width: 4
#| fig-height: 4
#| fig-cap: "Joint posterior distribution of lapse rate (mixture 1
#|   weight), bias (mixture 2 mean), and psychometric model mean."
d.draws <- get_samples_from_mixture_model(model = fit.lapse.mixed.mixture)

scatterplot3d(
  d.draws$b_theta1_Intercept, d.draws$b_mu1_Intercept, d.draws$b_mu2_Intercept,
  pch = 16, angle = 55, color = adjustcolor("black", alpha.f = .015),
  xlab = "b_theta1_Intercept", ylab = "b_mu1_Intercept",
  zlab = "b_mu2_Intercept")
```

```{r, warning=FALSE}
#| label: fig-3d-gamma-lambda
#| column: margin
#| fig-width: 4
#| fig-height: 4
#| fig-cap: "Joint posterior distribution of $\\gamma$, $\\lambda$, and psychometric model intercept."
scatterplot3d(
  d.draws$gamma, d.draws$lambda, d.draws$b_mu2_Intercept,
  pch = 16, angle = 55, color = adjustcolor("black", alpha.f = .015),
  xlab = "gamma", ylab = "lambda", zlab = "b_mu2_Intercept")
```

```{r, warning=FALSE}
#| label: fig-3d-lapse-slope
#| column: margin
#| fig-width: 4
#| fig-height: 4
#| fig-cap: "Joint posterior distribution of lapse rate parameter
#|   vs. mean and slope of the psychometric model."
scatterplot3d(
  d.draws$b_theta1_Intercept, d.draws$b_mu2_Intercept, d.draws$b_mu2_Size,
  pch = 16, angle = 55, color = adjustcolor("black", alpha.f = .015),
  xlab = "b_theta1_Intercept", ylab = "b_mu2_Intercept", zlab = "b_mu2_Size")
```

Finally, the predictions for each of 100 random posterior samples from each subject are shown in @fig-mixture-results. The solid lines show the predictions based on the *means* of the posterior samples. This line does not have to fall in the center of the other lines because the three parameters of the model can be correlated (within and across participants). Taking the mean, rather than marginalizing over the entire posterior distribution, ignores those correlations (but is a *lot* faster to compute and thus used for the present purpose). The parameter summaries at the top left of each graph show those mean parameter values---i.e., those are the parameters of the solid line:

```{r, fig.width=8, fig.height=16, message=FALSE}
#| label: fig-mixture-results
#| fig-cap: "Psychometric predictions for all subjects under the
    ...
    { if (!animate) 
      geom_line(
        data = . %>% 
          group_by(Subject, Size, Condition) %>%
          summarise(
            b_mu2_Intercept = mean(b_mu2_Intercept +
                                   b_mu2_Condition * Condition.num),
            b_mu2_Size = mean(b_mu2_Size + b_mu2_SizeCondition * Condition.num),
            gamma = mean(get_gamma(theta = b_theta1_Intercept,
                                   mu = b_mu1_Intercept)),
            lambda = mean(get_lambda(theta = b_theta1_Intercept,
                                    mu = b_mu1_Intercept)),
    ...
    geom_text(
      data = . %>% 
        { if (!animate) {
          ...
          mutate(., 
                 b_mu2_Intercept = b_mu2_Intercept +
                   b_mu2_Condition * Condition.num,
...
p <- suppressMessages(suppressWarnings(plot_psychometric(model = fit.lapse.mixed.mixture, animate = F))) 
suppressMessages(suppressWarnings(plot(p)))
ggsave(
  plot = p,
...
```{r, message=FALSE, warning=FALSE}
#| label: fig-nlm-unconverged
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Conditional predictions from the unconverged non-linear model fit for Subject 1."
p <- suppressMessages(suppressWarnings(
...
```{r, message=FALSE, warning=FALSE}
#| label: fig-nlm-constrained
#| column: margin
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "Conditional predictions from the constrained non-linear model fit for Subject 1."
p <- suppressMessages(suppressWarnings(
