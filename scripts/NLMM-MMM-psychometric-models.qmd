...
```{r set-options, include=F, cache=FALSE}
library(knitr)
opts_chunk$set(
               comment="", 
...
library(gifski)
library(scatterplot3d) # static 3D scatter plots (render in PDF, unlike plotly)
```

```{r constants, include=F, cache=FALSE}
chains = 4
options(
  width = 80,
...
```{r functions, include=F, cache=FALSE}
...
# the lapse rate, the guess (bias), and the linear predictor eta. Each of 
# these components can be described by a linear predictor. For this 
# psychometric model we describe eta as the linear combination of the 
# intercept and the effect of the stimulus. The other two parameters are 
# just described by only their respective 'intercepts'.
BF <- brmsformula(
...
```{r, echo=T}
# We store the mean and sd of Size so that we can later transform the
# model's predictions back onto the scale of original Size predictor
Size.mu <- mean(d$Size)
Size.sd <- sd(d$Size)
...
```{r simple-example-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
fit.lambda0.1 <- brm(
  BF.lambda,
  data = d %>%
    filter(Subject == "1"),
...
```{r simple-example-results, message=FALSE, warning=FALSE}
...
```{r simple-example-threshold, warning=F}
...
Here is an example draw from the data generator with 100 data points per stimulus at five different stimulus locations between 0 and 3 for $\lambda = .05$. The fit under the assumption of zero lapses is shown in @fig-sim-fit-nolapse.
...
Next, the ground truth functional relation including the lapse rate is shown in @fig-sim-ground-truth.
...
And, finally, the perceptual model comparison between the ground truth ($\lambda = .05$, gray) and the zero-lapse assumption (blue) is shown in @fig-sim-perceptual-only.
...
estimates = 
  update(analyses[[1]], newdata = d.temp, chains = 4, cores = 4,
         backend = "cmdstanr") %>% 
  fixef() %>% 
  as_tibble() %>% 
  pull(Estimate)
...
```{r simulation-results-plots-correlations-1, warning=FALSE}
...
```{r simulation-results-plots-correlations-2, warning=FALSE}
...
```{r plotting-simulation-plots-results-1, warning=FALSE}
...
```{r plotting-simulation-plots-results-2, warning=FALSE}
...
```{r subject3-lapse0-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
my.priors <- c(
  prior(student_t(3, 0, 2.5), class = "b", nlpar = "eta"))
...
fit.nl.s3.lambda0 <- brm(
  ...
  data = d %>%
    filter(Subject == "3"),
  prior = my.priors,
  # sample more posterior samples and thin to deal with correlations
  iter = 4000,
  thin = 5,
  control = list(adapt_delta = 0.999),
  # suppress sampling update messages
  refresh = 0,
  silent = 2,
  file = "../models/subject3-lambda0")
```

```{r subject3-lapse0-summary, message=FALSE, warning=FALSE}
my_hypotheses(fit.nl.s3.lambda0)
```

```{r fig-s3-lapse0, message=FALSE, warning=FALSE}
#| label: fig-s3-lapse0
...
```{r subject3-lapse05-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r subject3-lapse05-summary, message=FALSE, warning=FALSE}
my_hypotheses(fit.nl.s3.lambda0.05)
```

```{r fig-s3-lapse05, message=FALSE, warning=FALSE}
...
```{r subject3-lapse2-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r subject3-lapse2-summary, message=FALSE, warning=FALSE}
my_hypotheses(fit.nl.s3.lambda0.2)
```

```{r fig-s3-lapse02, message=FALSE, warning=FALSE}
...
```{r subject3-lapse-inferred-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
my.priors <- c(
  prior(student_t(3, 0, 2.5), class = "b", nlpar = "eta"),
...
```{r subject3-lapse-inferred-summary, message=FALSE, warning=FALSE}
my_hypotheses(fit.nl.s3.lambda)
```

```{r fig-s3-lapse-inferred, message=FALSE, warning=FALSE}
...
BF.lambda.mixed <- brmsformula(
  # reparameterizing the model by fitting lapses in log-odds space
  # (in order to aid convergence)
...
```{r fit-lambda-mixed-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# note that I am not constraining the values of lambda here.
# But that could be achieved with the
...
```{r fit-lambda-mixed-summary, message=FALSE, warning=FALSE}
...
```{r lambda-animation-frames, include=FALSE, fig.width=8, fig.height=14, fig.show='hold', dev='png', warning=F, message=F}
...
```{r fig-3d-lambda-correlations, warning=FALSE}
...
```{r mixture-GLMM-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r mixture-GLMM-summary, message=FALSE, warning=FALSE}
...
Next, we visualize the joint posterior distribution of the parameters of the model. The joint posterior distribution of the lapse rate (mixture 1 weight), bias (mixture 2 mean), and mean of the psychometric model (mixture 2) is shown in @fig-3d-mixture-correlations.
...
The joint posterior distribution of $\gamma$, $\lambda$, and the psychometric model intercept is shown in @fig-3d-gamma-lambda. As shown, most parameter values lie very close to the boundary at 0, illustrating why sampling $\gamma$ and/or $\lambda$ directly is difficult:

```{r fig-3d-gamma-lambda, warning=FALSE}
...
The lapse rate vs. mean and slope of the psychometric model are shown in @fig-3d-lapse-slope:

```{r fig-3d-lapse-slope, warning=FALSE}
...
And as an animation to illustrate the covariation in the psychometric functions across subjects:

```{r lapse-animation-frames, include=FALSE, fig.width=8, fig.height=14, fig.show='hold', dev='png', message=FALSE}
...
\begin{figure}[H]
\centering
\animategraphics[width=0.8\linewidth,controls,loop]{10}{NLMM-MMM-psychometric-models_files/figure-pdf/lapse-animation-frames-}{1}{100}
\caption{Posterior draws illustrating covariation in psychometric functions across subjects.}
\end{figure}
```
...
```{r nlm-unconverged-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
BF <- brmsformula(
  ResponseCorrect ~ gamma + (1-gamma-lambda) * inv_logit(eta),
  eta ~ 1 + Size,
  gamma ~ 1,
...
```{r nlm-unconverged-summary, message=FALSE, warning=FALSE}
summary(fit.1)
```

```{r fig-nlm-unconverged, message=FALSE, warning=FALSE}
...
```{r nlm-constrained-fit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
fit.2 <- brm(
  BF,
  data = d %>%
    filter(Subject == "1"),
...
```{r nlm-constrained-summary, message=FALSE, warning=FALSE}
summary(fit.2)
```

```{r fig-nlm-constrained, message=FALSE, warning=FALSE}
