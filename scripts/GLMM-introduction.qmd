...
subtitle: "Model comparison and mediation analyses"
...
::: {#fig-glmm-overview .column-margin}
```{=latex}
\centering
...
\end{minipage}
```
The generalized linear model (GLM, **top**) and generalized linear mixed-effects/multilevel model (GLMM, **bottom**). For the GLMM, $z_j$ is the $j$th level of the grouping variable $z$, which selects which $\beta_j$ is relevant for the present case (see homework for details).
:::

# Overview
This document is intended as in-class material. Make sure to first work through the [homework](https://github.com/tfjaeger/Tutorial-GLMM/scripts/GLMM-introduction-in-preparation-for-class.pdf) assigned as preparation for this class. 
...
We continue to work through the same data set as in the homework.
...
Here is a possible write-up of the problem set described in the homework. I highlighted a few parts that not everyone had quite right (minor typos are not italicized). 

\begin{quote}
Figure XXX shows the proportion of correct responses by number size and crowdedness condition. We analyzed \textit{trial-level} data with mixed-effects logistic regression using the function `glmer` from the lme4 package (Bates et al., 2013) in R (R Core Team 2020). Correct responses (correct response = 1 and an incorrect response = 0) were regressed against letter size (centered), condition (deviation coded, where .5 = crowded and -.5 = uncrowded) and their interaction. Following standards of the field, we fit the maximal random effect structure required by the design, random by subject intercepts and slopes for condition, size, and their interaction. \textit{The model converged but indicated a possible singular fit. Since additional results with reduced random effect structures confirmed the qualitative results of the model with the full random effect structure, we report the latter.} 
...
::: {#fig-bayesian-graphical-model .column-margin}
```{=latex}
  \centering
\tikz{ %
    \node[obs] (outcome) {$y_{i,j}$} ; %
    ...
    \factor[above=of Sigma-prior-tau] {Sigma-prior-tau-dist} {above:$\mathcal{Cauchy(0, 2.5)}$} {} {} ; %
    \factor[right=of Sigma-prior-rho] {Sigma-prior-rho-dist} {below: $\mathcal{LKJ(1)}$} {} {}; %
    \node[latent, right=of group] (beta) {$\beta$} ; %
    \factor[below=of beta] {beta-prior-dist} {below: $\mathcal{Student(0, 2.5, 3)}$} {} {} ; %
    \node[obs, right=of X, above=of betas] (grouplevel) {$z_j$} ; %
    % plates
    \plate[inner sep=0.12cm, xshift=-0.06cm, yshift=0.06cm] {plate1} {(mu) (distribution) (link) (X) (outcome)} {$N $}; %
    \plate[inner sep=0.12cm, xshift=-0.06cm, yshift=0.06cm] {plate2} {(mu) (distribution) (link) (betas) (X) (outcome) (grouplevel) (plate1) } {$M $}; %
...
```
Bayesian Generalized linear mixed model (GLMM) with weakly regularizing priors: a uniform distribution over the correlation matrix of the random effects (R), a Cauchy distribution with location parameter 0 for the standard deviations of the random effects ($\tau$), and a Student $t$ distribution with location 0 for the fixed effects ($\beta$). The split of the variance-covariance matrix of the random effects $\Sigma$ into a correlation matrix and a vector of standard deviations is not strictly necessary but since it is often used in the actual implementation of GLMMs, we follow it here.
:::
...
```{r, echo=T}
# Store the mean and sd of Size so that we can later transform the model's predictions back onto the original Size scale.
Size.mu <- mean(d$Size)
Size.sd <- sd(d$Size)

# Standardize Size and ensure ordering of Condition is as intended.
d %<>%
  mutate(
    Condition = 
      factor(
...
```{r, echo=T, fig.height=3, fig.width=4, fig.show="hold", out.width="33%", cache=FALSE}
bm <- brm(
...
```
@fig-bayesian-conditional-effects-condition and @fig-bayesian-conditional-effects-interaction show the predictions of the model. We can use Bayesian hypothesis tests to obtain a measure of the support for/against two competing hypotheses. For example, to assess the support for the hypothesis that increasing letter size results in better performance (against the hypothesis of the *opposite* effect, including a zero effect):

```{r, echo=T}
#| column: body
hypothesis(bm, "Size > 0")$hypothesis %>%
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>%
  kable(format = "latex", booktabs = TRUE, digits = 3, linesep = "")
```

```{r}
#| label: fig-bayesian-conditional-effects-condition
#| warning: false
#| message: false
#| column: margin
#| fig-width: 3.5
#| fig-height: 3
#| fig-cap: "Bayesian conditional effect of Condition."
plot(conditional_effects(bm, effects = "Condition"), ask = F)
```

```{r}
#| label: fig-bayesian-conditional-effects-interaction
#| warning: false
#| message: false
#| column: margin
#| fig-width: 3.5
#| fig-height: 3
#| fig-cap: "Bayesian conditional effect of the Size by Condition interaction."
plot(conditional_effects(bm, effects = "Size:Condition"), ask = F)
```
...
```{r, echo=T}
#| column: body
hypothesis(bm, "ConditionCrowded.vs.Uncrowded < 0")$hypothesis %>%
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>%
  kable(format = "latex", booktabs = TRUE, digits = 3, linesep = "")

hypothesis(bm, "Size:ConditionCrowded.vs.Uncrowded < 0")$hypothesis %>%
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>%
  kable(format = "latex", booktabs = TRUE, digits = 3, linesep = "")
```
...
2. We can visualize the correlation between the posterior samples of different fixed effect parameters. As shown in @fig-bayesian-correlation-matrix, what do you make out of these correlations?

```{r}
#| label: fig-bayesian-correlation-matrix
#| column: margin
#| warning: false
#| message: false
#| fig-width: 3.5
#| fig-height: 4.5
#| fig-cap: "Posterior correlation structure among the Bayesian fixed effect parameters."
bm %>%
  spread_draws(
    b_Intercept,
    b_Size,
  ...
  geom_autodensity() +
  geom_density2d() +
  geom_smooth(method = "lm", color = "red", linewidth = .5) +
  facet_matrix(
    vars(everything()),
    layer.diag = 2,
    layer.upper = c(3, 4),
