---
title: "Categorical predictors in regression analysis"
subtitle: "Factor coding and its interpretation in linear regression"
...
# Reading and preparation for this class
Please read Gelman et al. "Regression and Other Stories" (ROS) Ch. 7.3. If you want to challenge yourself, go through problem sets 7.1 and 7.9. Then read and *work through* this document. We will use class to go through the important concepts and to address any questions that you have about the readings or the problem sets in this document. 
...
**Optionally**, you can read Schad, Vasishth, Hohenstein, & Klieg (2020). It is an exhaustive summary of different contrast coding schemes and their implications for regression analysis. It's rather small-stepped and really walks you through all kinds of different scenarios you might encounter. But it's also very long. In any case, I would definitely store it in your library for later reference---it's the type of paper you might want to look up when you get to analyze your own data and find yourself wondering how you should code your variables. Another good reference is James et al. (2013, Ch 3.3 up to but *not* incl. 3.3.3). 
...
If we want to include qualitative/categorical predictors (i.e., *factors*, e.g., "Condition A" vs. "Condition B") in an LM, we need to translate these predictors into numerical variables.[^0] This process is sometimes called predictor or factor *coding*. Some people use this term narrowly to only refer to the coding of categorical predictors. Others use it to refer to any changes or transformation we make to our predictors, including continuous predictors (e.g. centering or scaling). In this tutorial, we talk about both.

[^0]: In R, the `contrasts()` function and the family of `contr.*`
    functions are very helpful for factor coding (a process also known as contrast coding),
    without having the manually transform columns in your data table into numerical values. This
...
```{r, echo=F}
#| label: fig-condition-threshold
#| column: margin
#| fig-cap: "Size (in arcminutes) at which threshold performance (62.5% correct) was reached by crowding condition. Each color shows a separate subject. Black cross and line show the predicted outcome of a linear model fit to the data across subjects."
#| fig-width: 4
#| fig-height: 4
d %>%
...
### Prepare for class
 ...
 1. What happens if we double the values we assign to each of the two effect-coded conditions, i.e., if we use 1 vs. -1 instead of .5 vs. -.5? What if we set them to -1000 vs. 1000? What changes and what doesn't? Why? (Try it)

```{r, echo = T}
contrasts(d$Condition) = cbind("Crowded vs. mean" = c(1,-1))
```

 2. Are the predicted/fitted values of any of these effect-coded models different from each other? (Try it)
 3. Are the predicted/fitted values of any of these effect-coded models different from the treatment-coded model presented in the previous section? Why?
 

## Writing up results

How would we write up, for example, the effect-coded model? There are, of course, about as many difference preferences as they are researchers, and so you will find conflicting advice. But generally, it will be helpful if you are clear about all of the following:
