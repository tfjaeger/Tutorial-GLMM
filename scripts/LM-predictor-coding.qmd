...
subtitle: "An applied tutorial on factor coding and its interpretation for continuous and categorical predictors in linear regression"
...
**Please note that this document provides R code only.** Some of the steps described here might have less direct solutions in Matlab, so it is recommended that you start early. I have, however, tried to describe each step in a way that does not depend on R or Matlab. 
...
[^0]: In R, the \texttt{contrasts()} function and the family of \texttt{contr.*}
