...
A generalized linear model extends the linear model (LM) to outcome variables drawn from---or *assumed* to be drawn from---the exponential family of distributions. Concise introductions are provided in Gelman & Hill (2007, Ch. 6) or McElreath (2019, Ch. 10.2). 
...
## Lik'em Bayesian? Adding priors

What would change if we want to go Bayesian? Here I focus on the formulation of the model (but there are also differences in how the model is fit, and the output). Essentially, all we need to do is to add *priors* to all latent variables (free parameters) in the model (e.g., the $\beta$s). These priors can be seen as encoding our (the researchers') expectations about what would constitute plausible values for the parameters prior to having seen the data we wish to analyze. Fitting the model to the data will then update these prior beliefs/distributions, based on the data, to the posterior beliefs/distributions of the parameters. Those posterior distributions of the free parameters are what we base our statistical inferences and interpretations of the data on.

We might set priors based on previous research, or based on a specific hypothesis. Those would be *informative* priors. Most often, however, research in the cognitive and neurosciences selects *uninformative* priors. Specifically, standard recommendations for Bayesian GLMM analyses often suggest *weakly regularizing* priors. Here, "regularizing" means that the prior has most probability mass over the null hypothesis, essentially 'pulling' the posterior estimate towards the null (i.e., being conservative). And "weakly" means that the prior is weak (i.e., wide), so that this 'pulling' towards zero is relatively gentle. Here's the resulting graphical model for GLMMs assuming weakly regularizing priors.^[Note that the strength of priors for, e.g., $\beta$s thus to be interpreted relative to the scale of the predictor. This is one of several reasons why it is recommended that all predictors are put on the same scale. Here we assume that all predictors are Gelman scaled (Gelman, 2008) by centering them and dividing them through twice their standard deviation.]
...
The remainder of this homework uses frequentist GLMMs.
...
For this tutorial we are continuing to use Ashley Clark's data from her experiment on visual crowding effects on foveal processing. \textbf{Do not share this data without Ashley's permission. Thank you.}
