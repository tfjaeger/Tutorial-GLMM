...
to do: illustrate random effects.
...
plot_model(
  ...
  theme_fivethirtyeight()
```
The so-called best unbiased linear predictors (BLUPs)---the posterior mode of the random effects for each subjects---can also be extracted from the model. Whereas the points in the above plot reflect the mean of the by-subject effect estimated for each subject, the BLUPs are the mode of that distribution (the value with the highest estimated density). Typically, these two values will be closely related. Take, for example, the BLUP for subject 2's intercept (.7317...) and compare it to the plot (second to last row, last column).
