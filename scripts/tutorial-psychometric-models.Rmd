...
```{r generate-data-generator, echo=T}
...
```
For our simulation, we obtain 100 simulated data sets for each of a variety of combinations of values for the true intercept, slope, and lambda (gamma is always .5 since we're simulated a 2AFC task). e will consider four different analyses, each assuming a different fixed lapse rate (0, .025, .05, .1). 
...
```{r plot-simulation-results-correlations}
p = d.sim %>%
...
```{r, echo=T}
my_hypotheses = function(model) {
  print(hypothesis(model, "eta_Size > 0"))
  print(hypothesis(model, "eta_ConditionCrowded.vs.Uncrowded > 0"))
  print(hypothesis(model, "eta_Size:ConditionCrowded.vs.Uncrowded > 0"))
}
