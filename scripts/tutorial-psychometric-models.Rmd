...
my_hypotheses = function(model, prefix= "eta") {
  hypothesis(model, paste0(prefix, "_Size > 0"))$hypothesis %>%
    ...
    { if (paste0("b_", prefix, "_ConditionCrowded.vs.Uncrowded") %in% parnames(model)) 
      rbind(
        ., 
        rbind(    
    ...
    { if (any(c("b_lambda_Intercept", "b_lapse_Intercept") %in% parnames(model))) 
      rbind(
        ., 
        hypothesis(
          model, 
          ...
          mutate(Hypothesis = "$\\lambda > 0$")) else . } %>% 
...
To learn more about psychometric models, you might also find Gilchrist et al. (2005) and Wichmann and Hill (2001a, b) helpful (both in *Perception & Psychophysics*). Prins (2013, *Journal of Vision*) provides a counterpoint to the solution proposed in Wichmann and Hill (2001). All of these papers are relatively heavy on math but the Prins reading is written in a comparatively accessible style. 
...
Four commonly used perceptual models include the cumulative Gaussian, logistic, Weibull, and Gumble (Generalized Extreme Value distribution a.k.a. log Weibull). For formal introductions, the relation between the different functions, and their relative trade-offs, see the readings listed at the top of this tutorial. Of relevance here is that any of these perceptual models can be expanded to account for the proportion of trials on which the subject responds independent of the stimulus (e.g., because the subject experienced an attentional lapse) and optionally the bias (or guessing rate) on those lapsing trials. 

# Lapsing model (Wiechmann and Hill formulation)

The lapse model can specified in two different ways. First, in the Wiechmann and Hill (2001) formulation:
...
We use the same data as in the GLMM tutorial from Ashley's experiment. Please consult the GLMM tutorial for a detailed description of the experiment. As a reminder, here is an overview of the data:
...
We fit the model with the function \texttt{brm()} from the library \texttt{brms}, and summarize the output:
...
For our simulation study, we continue to consider a simple model as in the previous section, with only the intercept and an effect of the stimulus (rather than considering additional conditions). We first make a data generator for a 2AFC task that creates data following the Wiechman and Hill (2001) model with a logistic response function:
...
BF.lapse.mixed <- brmsformula(
  ...
  eta ~ 1 + Size * Condition (1 + Size | Subject),
  lapse ~ 1 + (1 | Subject), 
  family = bernoulli(link="identity"),
  nl = TRUE
)
...
And the joint posterior distribution of the model's three fixed effect parameters (and could do similarly for each subject). This reveals a correlation between the intercept and slope of the model. To view this interactive plot you might have to compile (knit) this document to HTML (rather than PDF) or go through the code incrementally and then execute just this code chunk.
...
When inferring both $\gamma$ and $\lambda$ is substantially more efficient (and often necessary) to switch to the mixture formulation of the mode, as it allows us to orthogonalize $\gamma$ and $\lambda$. This makes this formulation of the model for computationally efficient, and more likely to converge. The mixture specification is, however, only available if the distributional family of the perceptual model is defined over real numbers (like the lapse part of the model). This is not, for example, the case for the Weibull (and hence Gumbel) model, which is defined over counts (integers). 
...
We can use animation to further illustrate the hierarchical nature of the model. Note how the different posterior samples (each animation step) 'move in sync'. That is because the subject-specific predictions include the population-level predictions that are common to all subjects:
