...
# The linear model (LM)
LMs (aka linear regression) can be thought of as an example of a much larger family of models that describe an *outcome* variable $y$ (aka the *dependent* variable) as the function $g$ of one or more *predictor* variables $x_1$, ..., $x_k$ (aka *independent* variables), $y = g(x_1, ..., x_k)$. In this sense, the LM is a---very constrained---*predictive model*. Specifically, the LM assumes that $y$ is a draw from a Normal distribution $\mathcal{N}$ with constant standard deviation $\sigma$ and a mean $\mu$ that is a *linear combination* (the weighted sum) of all the predictors included in the model:
...
In the literature, LMs are often written in a number of different ways. For instance, one might describe the linear model for each observation $i$ of the outcome $y_i$ and all predictors $x_{1,i}, ..., x_{k,i}$:^[$\epsilon_i$ is the *residual* for observation $i$ (observed $y_i$ - predicted $\hat{y_i}$).]

\begin{align}
y_i & = & \beta_1x_{1,i}+...+\beta_kx_{k,i} + \epsilon_i, & \ \epsilon_i \sim \mathrm{Normal}(0, \sigma_{resid}) 
\end{align}

or in vector form, or for as in James et al. (2013, p. 63):
...
or combining equations (1) and (3) from above, as in Gelman & Hill (2007, p. 38):

\begin{align}
\mathbf{y} & = & \mathrm{Normal}(X\beta, \sigma_{resid}) &  &  & 
\end{align}

Any of these formulations are equivalent, and all are used in the literature. Finally, a common---but arbitrary---convention is to use a different symbol for one particular predictor, the *intercept* $\alpha$: 
...
As a theoretical model the LM makes several assumptions (cf. Gelman \& Hill, 2007, p. 45). The first two assumptions are shared with many/most other statistical models we use (to be precise, some models don't strictly make these assumptions, but they do correct for their violations):
 ...
 + **Independence of observations/errors**. Each observation $y$ is assumed to be drawn independent of all other observations.
 + **Validity/exhaustivity**. 
 
The other assumptions are more specific to the LM, though the last two of these are shared with GLMs and GLMMs, as we'll see in later classes:^[The last two assumptions often confuse learners. To understand them it is important to distinguish between **input variables** and the **predictors** we derive from them. For example, in the case study presented below, one of the input variables is $VOT$. But we could include both $VOT$ and $VOT^2$ as predictors in the model. This would model both linear and quadratic effects of VOT, each of which is assumed linearly add to the predicted outcome $y$. Similarly, we can include interactions of the input variable $VOT$ and some other predictor in the model, which will add their product as a predictor to the model. That is, we can very much use an LM to ask whether two variables have super- or sub-additive interactions. The additivity assumption just means that the effects of the interaction predictor are assumed to be added to the effects of the other predictors in the model.] 

 + **Normality of errors**. $y$ is drawn from a Normal distribution $\mathcal{N}$ (with mean $\mu = X\beta$), and thus is assumed to be able to take any real value from $-\infty$ to $+\infty$. Equivalently, we can say that the *residuals* (or prediction errors) are drawn from a Normal distribution (with mean 0).
 + **Equal variance of errors**. While ${\rm E[y] = \mu}$ is assumed to vary with the predictors $X$, the residual standard deviation around that mean $\sigma$ is assumed to be independent of $X$, and thus constant and identical across all observations. 
 + **Additivity of effects**. The *effects* of predictors are assumed to be additive: ${\rm E}[y]$ is assumed to be the weighted *sum* of the predictors.
 + **Linearity of each effects**. $y$ is linear in the $\beta$s. That is, the *effects* of predictors are assumed to be linear.
 
Many, if not most, of these assumptions are actually violated when we apply the model. To some extent linear regression is robust to such violations, depending on the specific assumption. For example, instead of equality of variance it is often sufficient that variances are similar (*homogeneity* of variance) or that the variance-covariance matrix does not exhibit *heteroscedasticity*.^[From Ancient Greek [*hetero*---"different" and *skedasis*---"dispersion"](https://en.wikipedia.org/wiki/Heteroscedasticity). Homoscedasticity extends the notion of homogeneity of variance to the *co*variance between variables] **But violations of the independence, normality, or homoscedasticity assumptions can make a model invalid, and the statistical inferences based on it invalid** (e.g., by inflating Type I and II errors). We return to these points in later classes.
...
This output can be split into four parts. At the top, the model call is repeated, which is convenient, for example, when we look at a stored model object that we created some time ago. Below it, we see a statistical summary of the residuals.^[**Residuals** are the prediction error of the LM, the difference between the actual and the predicted outcome. Each row of the model matrix $X$ corresponds to a prediction for that row, and thus has a corresponding residual $\epsilon$.] This summary can be helpful in identifying violations of the assumption of normality. Here, for example, there is clear summary that the residuals are *not* normally distributed: the minimum/maximum and quantiles are not symmetrical around zero. This is to be expected---but often ignored---for RTs, which are known to be right-skewed with heavy right tails (due to the boundedness of RTs).^[For review and large-scale statistical simulations that assess the consequences of using LMs and its repeated-measures extension LMMs for the analysis of RTs, see Burchill & Jaeger (2024)-*JML*.] 

The third part of the output summarizes the coefficient estimates ($\hat{\beta}$), their standard errors, the resulting $t$-statistic, and its $p$-value. The $t$-statistics is a signed measure of the estimates distance from 0 in terms of its own standard error ($t = \frac{\hat{\beta}}{SE_{\hat{\beta}}}$).^[This is just the [standard definition of a $t$-value](https://en.wikipedia.org/wiki/T-statistic) as a measure of how different an estimate is from some target value. Here, the target value is 0 since we want to know whether the coefficient for a predictor is different from 0. If it is, we say that the predictor "has an effect" (on the outcome).] The $p$-value is the result of a Student's $t$-test with the residual DFs of the LM as DFs for the test. E.g., for the effect of VOT, $t(70) = `r coef(summary(m))[2,3]`$. And if we look the probability density function for the Student $t$ distribution to ask how probable this $t$-value is for DF = 70, we'll get `r pt(q = abs(coef(summary(m))[2,3]), df = m$df.residual, lower.tail = F) * 2`, which---after rounding---matches the $p$-value reported in the LM for the effect of VOT. 

Finally, the bottom part of the LM output provides multiple measures of the goodness of fit of the LM. Of these, the $R^2$ tells us how much of the variance in the outcome $y$ is accounted for by the model (*for this sample*). Notably, an LM can have one or more statistically significant effects even if the model overall accounts for very little variance in out observed outcome. Just as the size of an effect is not to be confused with its theoretical relevance, the $R^2$ of a model is not to be confused with whether it is a good model. Many effects of high theoretical relevance might be very small and difficult to detect.^[Of course, if our goal is to predict a certain human behavior---like an election, or whether someone will choose the red or blue pill---we might indeed only be interested in models that account for much of the observed variance. But keep in mind that we routinely aim to massively *reduce* natural variance in our experiments, e.g., by removing other influences. So be careful not to equate variance explained in a particular experiment with variance in human behavior explained.] The $F$-statistic (here $F(1, 70) = 2.779$) tests whether the predictors included in the model *together* provide for a statistically significantly better fit, compared to an LM with just the intercept (the "null model"). The DFs for this test will always be the number of predictors in the model. We will return to this $F$-test when we talk about model comparison.
...
\color{lightgray}
> To test our hypothesis, we used linear regression to regress the participant's reaction time against VOT. The effect of VOT was not significant ($\hat{\beta} = -7.6$, $t = -1.67$, $p \geq .1$).

\color{black}
A few considerations:
 
 + I recommend you provide both the relevant statistics (coefficient estimates, $p$-values, etc.), and a descriptive interpretation of that result in non-technical language---but we careful to avoid language that is wrong. 
 + **Use $\widehat{\beta}$ when we describe coefficient estimates and write up results**. This emphasizes that these are *estimates* of the assumed population parameters ($\beta$).
...
# Fitting
Consider the following fitted LM from the previous section:
...
```{r, , fig.show='animate', fig.cap="Raw RTs for one participants across the six test blocks (points), along with prediction line for different intercepts {\\em a} and slopes{\\em b} for the model \\texttt{RT} $\\sim$ \\texttt{1 + VOT}. Compare how the RSS changes, depening on {\\em a} and {\\em b}.", out.width="80%"}
...
While minimization of RSS works well for LMs, there is a more generally applicable approach that yields the same results for LMs but also can be used for the fitting of many others types of models, *likelihood maximization*. Under this approach, we find the best-fitting estimates that maximize the likelihood of the observed outcomes $y$. Recall that the LM assumes $y$ to be normally distributed around the predicted ${\rm E}[y]$. This means we can calculate the *likelihood* of each observation under the hypothesized normal distribution. This normal distribution is a function of the hypothesized values for all the model's coefficients ($a$ and $b$ in the example above) *and* the value hypothesized for $\  sigma$. For any hypothesis about $a$, $b$, and $\sigma$, we can get the density of an observation $y$ under $\mathcal{N}(a + bx, \sigma)$. By multiplying the densities of all observations, we can get the data likelihood under the hypothesized values for $a$, $b$, and $\sigma$. In practice, we instead sum the log-transformed likelihoods. This is numerically more stable, and the $x$ that maximizes $\log(f(x))$ is the same that maximizes $f(x)$.
 ...
 + Define a function that calculates the sum of log-densities for the model \texttt{RT} $\sim$ \texttt{1 + VOT} given a set of arguments $a$, $b$, and $\sigma$.^[In R, \texttt{dnorm()} will give you the density of an observation or vector of observations $y$ under a Normal with the specified mean and standard deviations. Use the optional argument \texttt{log = TRUE} to get log densities.]
 ...
 + To further understand the relationship between OLS and ML fitting of linear regression with normally distributed residuals, see [this presentation, slides 99-122](https://faculty.washington.edu/cadolph/mle/topic2.p.pdf] (it's a lot of slides but that's just because each derivation step is presented as a separate slide).
